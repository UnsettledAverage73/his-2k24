#!/usr/bin/env bash
{
  ...
  if [ "$l_hbfw" = "nft" ]; then
    # Check if loopback traffic is set to accept
    ...
    # Check if IPv4 traffic from the loopback address is dropped
    ...
    # Check if IPv6 traffic from the loopback address is dropped

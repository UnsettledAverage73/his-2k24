...
if command -v cron >/dev/null 2>&1 && [ -d /etc/cron.hourly ]; then
    # Get the current permissions, UID, and GID of /etc/cron.hourly
    ...
    if [ "$current_permissions" == "$expected_permissions" ] && [ "$current_uid" == "$expected_uid" ] && [ "$current_gid" == "$expected_gid" ]; then
        echo "PASS: /etc/cron.hourly has correct permissions and ownership."
    else
        echo "FAIL: /etc/cron.hourly does not have correct permissions or ownership."

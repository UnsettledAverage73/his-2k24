...
def execute_script(script_name, execution_label, output_text, progress_bar, log_filename, report_filename):
    try:
        ...
        with open(report_filename, "w") as report_file:
            report_file.write(f"Script Name: {script_name}\n")
            report_file.write(f"Execution Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            report_file.write(f"Log File: {log_filename}\n")
            if result:
                report_file.write("Status: Success\n")
                execution_label.config(text="Execution completed successfully")
            else:
                report_file.write("Status: Completed with errors\n")
...
def on_execute_button_click(execution_label, output_text, progress_bar):
    ...
    report_filename = f"report_{script_basename}_{timestamp}.txt"

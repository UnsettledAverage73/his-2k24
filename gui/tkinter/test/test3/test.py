...
def execute_script(script_name, execution_label, output_text, progress_bar):
    log_filename = "script_execution.log"  # Log file name
    try:
        ...
        generate_pdf_report(log_filename, "report.pdf", script_name)
...
def generate_pdf_report(log_filename, report_filename, script_name):
    try:
        ...
        if not errors:
            c.drawString(50, y, "No errors found.")

        c.save()
    except Exception as e:
        print(f"Error occurred while generating the report: {str(e)}")

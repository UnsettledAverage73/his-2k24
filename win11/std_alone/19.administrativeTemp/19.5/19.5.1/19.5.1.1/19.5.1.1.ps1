# Title: CIS Control 19.5.1.1 (L1) - Check 'Turn off toast notifications on the lock screen'

Write-Host "Checking compliance for: CIS Control 19.5.1.1 (L1) - Turn off toast notifications on the lock screen"
    ...
    foreach ($sid in $sidList) {
        ...
        if (Test-Path -Path $regPath) {
            ...
            if ($currentValue -eq $expectedValue) {
                Write-Host "Compliant for user SID $($sid.Name): 'Turn off toast notifications on the lock screen' is set to Enabled." -ForegroundColor Green
            } else {

# Title: CIS Control 19.7.8.5 (L1) - Check 'Turn Off Spotlight Collection on Desktop'

Write-Host "Checking compliance for: CIS Control 19.7.8.5 (L1) - Turn Off Spotlight Collection on Desktop"
    ...
    foreach ($sid in $sidList) {
        ...
        if (Test-Path -Path $regPath) {
            ...
            if ($currentValue -eq $expectedValue) {
                Write-Host "Compliant for user SID $($sid.Name): 'Turn Off Spotlight Collection on Desktop' is set to Enabled." -ForegroundColor Green
            } else {

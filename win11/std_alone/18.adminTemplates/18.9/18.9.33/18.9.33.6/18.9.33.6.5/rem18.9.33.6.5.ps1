# remedi.ps1
# Script to set "Require a password when a computer wakes (on battery)" to Enabled
...
if ($value.$regValue -eq 1) {
    Write-Output "'Require a password when a computer wakes (on battery)' has been successfully set to Enabled."
} else {
    Write-Output "ERROR: Failed to set 'Require a password when a computer wakes (on battery)' to Enabled."
}

# remedi.ps1
# Script to set "Allow standby states (S1-S3) when sleeping (on battery)" to Disabled
...
if ($value.$regValue -eq 0) {
    Write-Output "'Allow standby states (S1-S3) when sleeping (on battery)' has been successfully set to Disabled."
} else {
    Write-Output "ERROR: Failed to set 'Allow standby states (S1-S3) when sleeping (on battery)' to Disabled."
}

# remedi.ps1
# Script to set "Allow network connectivity during connected-standby (on battery)" to Disabled
...
if ($value.$regValue -eq 0) {
    Write-Output "'Allow network connectivity during connected-standby (on battery)' has been successfully set to Disabled."
} else {
    Write-Output "ERROR: Failed to set 'Allow network connectivity during connected-standby (on battery)' to Disabled."
}

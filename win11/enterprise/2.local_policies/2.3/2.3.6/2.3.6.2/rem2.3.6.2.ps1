# Title: Remediation Script for 'Domain member: Digitally encrypt secure channel data (when possible)'
# Description: Configures the policy to 'Enabled' by setting the registry value.

Write-Host "Starting Remediation: Configuring 'Domain member: Digitally encrypt secure channel data (when possible)' policy" -ForegroundColor Yellow
...
try {
    ...
    if ($regValue -eq $desiredValue) {
        Write-Host "SUCCESS: 'Domain member: Digitally encrypt secure channel data (when possible)' is now set to 'Enabled'." -ForegroundColor Green
    } else {
        Write-Host "FAIL: Unable to set 'Domain member: Digitally encrypt secure channel data (when possible)' to 'Enabled'." -ForegroundColor Red
    }
} catch {

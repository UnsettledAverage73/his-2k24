# Title: Remediation Script for 'Domain member: Digitally sign secure channel data (when possible)'
# Description: Configures the policy to 'Enabled' by setting the registry value.

Write-Host "Starting Remediation: Configuring 'Domain member: Digitally sign secure channel data (when possible)' policy" -ForegroundColor Yellow

# Define the registry path and value
$regPath = "HKLM:\SYSTEM\CurrentControlSet\Services\Netlogon\Parameters"
$regValueName = "SignSecureChannel"
$desiredValue = 1
...
try {
    ...
    if ($regValue -eq $desiredValue) {
        Write-Host "SUCCESS: 'Domain member: Digitally sign secure channel data (when possible)' is now set to 'Enabled'." -ForegroundColor Green
    } else {
        Write-Host "FAIL: Unable to set 'Domain member: Digitally sign secure channel data (when possible)' to 'Enabled'." -ForegroundColor Red
    }
} catch {

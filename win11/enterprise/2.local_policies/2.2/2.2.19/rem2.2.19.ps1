# Title: Remediation Script for 'Deny log on locally' Policy
# Description: This script configures the 'Deny log on locally' policy to include 'Guests' as per CIS recommendations.

Write-Host "Starting Remediation: Configuring 'Deny log on locally' policy" -ForegroundColor Yellow
...
try {
    ...
    # Update the policy for 'Deny log on locally'
    Write-Host "Updating the policy to include 'Guests'..." -ForegroundColor Cyan
    $updatedContent = $policyContent -replace '(?<=SeDenyInteractiveLogonRight = ).*', ($expectedAccounts -join ',')
    ...
    Write-Host "SUCCESS: 'Deny log on locally' policy is now configured correctly." -ForegroundColor Green
} catch {

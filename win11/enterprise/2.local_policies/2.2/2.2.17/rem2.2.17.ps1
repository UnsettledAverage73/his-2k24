# Title: Remediation Script for 'Deny log on as a batch job' Policy
# Description: This script configures the 'Deny log on as a batch job' policy to include 'Guests' as per CIS recommendations.

Write-Host "Starting Remediation: Configuring 'Deny log on as a batch job' policy" -ForegroundColor Yellow
...
try {
    ...
    # Update the policy for 'Deny log on as a batch job'
    Write-Host "Updating the policy to include 'Guests'..." -ForegroundColor Cyan
    $updatedContent = $policyContent -replace '(?<=SeDenyBatchLogonRight = ).*', ($expectedAccounts -join ',')
    ...
    Write-Host "SUCCESS: 'Deny log on as a batch job' policy is now configured correctly." -ForegroundColor Green
} catch {

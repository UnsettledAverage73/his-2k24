# Title: CIS Control 19.7.26.1 (L1) - Check 'Prevent Users from Sharing Files Within Their Profile'

Write-Host "Checking compliance for: CIS Control 19.7.26.1 (L1) - Prevent Users from Sharing Files Within Their Profile"
    ...
    foreach ($sid in $sidList) {
        ...
        if (Test-Path -Path $regPath) {
            ...
            if ($currentValue -eq $expectedValue) {
                Write-Host "Compliant for user SID $($sid.Name): 'Prevent Users from Sharing Files Within Their Profile' is set to Enabled." -ForegroundColor Green
            } else {

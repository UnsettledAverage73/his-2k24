# Title: CIS Control 19.7.8.1 (L1) - Check 'Configure Windows Spotlight on Lock Screen'

Write-Host "Checking compliance for: CIS Control 19.7.8.1 (L1) - Configure Windows Spotlight on Lock Screen"
    ...
    foreach ($sid in $sidList) {
        ...
        if (Test-Path -Path $regPath) {
            ...
            if ($currentValue -eq $expectedValue) {
                Write-Host "Compliant for user SID $($sid.Name): 'Configure Windows Spotlight on Lock Screen' is set to Disabled." -ForegroundColor Green
            } else {
